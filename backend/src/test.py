...
def make_runner(agent):
    ...
    asyncio.get_event_loop().run_until_complete(
        session_service.create_session(app_name="test-app", user_id="user", session_id="sess")
    )
...
@pytest.mark.parametrize("prompt,validate_fn", [
    ("Tell me something about horses.", lambda t: t in [
        "Horses cannot sleep.",
        "Horses have a unique way of communicating with each other through body language.",
        "The fastest recorded speed of a horse is 55 mph (88.5 km/h)."
    ]),
...
def test_api_integration(prompt, validate_fn):
    ...
    try:
        ...
        events = collect(runner.run_async(user_id="user", session_id="sess", new_message=content))
        # Extract text parts
        texts = [part.text for ev in events for part in ev.content.parts if hasattr(part, 'text')]
        assert texts, "No text returned from API integration test"
        answer = texts[-1].strip()
        assert validate_fn(answer), f"Validation failed for prompt '{prompt}', got '{answer}'"
